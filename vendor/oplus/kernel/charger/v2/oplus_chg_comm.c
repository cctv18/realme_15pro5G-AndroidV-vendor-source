...
static void oplus_comm_check_sw_full(struct oplus_chg_comm *chip)
{
	...
	chg_err("neutron dec_vol=%d spec->sw_fv_mv[chip->temp_region]=%d sw_fv_mv=%d", spec->dec_cv.dec_vol, spec->sw_fv_mv[chip->temp_region], sw_fv_mv);
...
static void oplus_comm_check_hw_full(struct oplus_chg_comm *chip)
{
	...
	chg_err("neutron dec_vol=%d spec->fv_mv[chip->temp_region]=%d spec->hw_fv_inc_mv[chip->temp_region]=%d",
			spec->dec_cv.dec_vol, spec->fv_mv[chip->temp_region], spec->hw_fv_inc_mv[chip->temp_region]);
	chg_err("neutron hw_fv_mv=%d", hw_fv_mv);
	if (!chip->authenticate || !chip->hmac)
...
#define TRACK_DEC_CV_MSG_LENTH 256
	...
	if (spec->dec_cv.dec_spec_support == DEC_CV_SUPPORT_LITE) {
		snprintf(msg_info, TRACK_DEC_CV_MSG_LENTH,
...
static void oplus_comm_ffc_start_work(struct work_struct *work)
{
	...
	vote(chip->fv_min_votable, OVER_FV_VOTER, true, 4630, false);
	vote(chip->fv_max_votable, FFC_VOTER, true, 4630, false);
	return;
...
err:
	...
	chip->ffc_fv_count = 0;
	chip->ffc_charging = false;
	vote(chip->fv_min_votable, OVER_FV_VOTER, true, 4520, false);
	vote(chip->fv_max_votable, FFC_VOTER, true, 4520, false);
	oplus_comm_set_ffc_status(chip, FFC_DEFAULT);
}
...
static void oplus_comm_check_ffc(struct oplus_chg_comm *chip)
{
	...
	case FFC_FAST:
		if ((ffc_temp_region < FFC_TEMP_REGION_PRE_NORMAL || ffc_temp_region > FFC_TEMP_REGION_NORMAL) && (chip->soc >= 100)) {
			chg_err("FFC charging is not possible in this temp region, temp_region=%s\n",
				oplus_comm_get_ffc_temp_region_str(ffc_temp_region));
		...
		if (((abs(chip->ibat_ma) < cutoff_ma) && (chip->soc >= 100 )) || sub_ibat_ffc_end_status)
			chip->ffc_fcc_count++;
		else
			chip->ffc_fcc_count = 0;
...
static int oplus_comm_parse_dt(struct oplus_chg_comm *comm_dev)
{
	...
	spec->use_pm_power_off_with_hightemp =
		of_property_read_bool(node, "oplus_spec,pm_power_off_with_hightemp");
		...
		if (get_eng_version() != HIGH_TEMP_AGING) {
			...
			if (oplus_is_power_off_charging() && chip->spec.support_hot_enter_kpoc) {
				...
				if (over_temp_count >= OPLUS_TBATT_HIGH_PWROFF_COUNT ||
					...
					machine_power_off();
#else
					if (chip->spec.use_pm_power_off_with_hightemp){
						pm_power_off();
					}
					else{
						kernel_power_off();
