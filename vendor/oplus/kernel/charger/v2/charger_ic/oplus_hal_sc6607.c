...
int sc6607_tsbus_tsbat_to_convert(struct sc6607 *chip, u64 adc_value, int adc_module);
...
static void sc6607_soft_bc12_work_func(struct work_struct *work)
{
	...
	chip = container_of(bc, struct sc6607, bc12);
	if (!chip)
		return;

	if (!chip || !bc12_should_run(chip))
		return;
...
static int sc6607_get_usb_btb_temp(struct oplus_chg_ic_dev *ic_dev, int *usb_btb_temp)
{
	...
	} else if (chip->sc6607_switch_ntc) {
		*usb_btb_temp = sc6607_get_tsbat(chip) / UNIT_TRANS_1000;
	} else {
...
static int sc6607_get_batt_btb_temp(struct oplus_chg_ic_dev *ic_dev, int *batt_btb_temp)
{
	...
	if (chip->batt_btb_temp_chan) {
		...
		*batt_btb_temp = sc6607_get_tsbus(chip) / UNIT_TRANS_1000;
	else
