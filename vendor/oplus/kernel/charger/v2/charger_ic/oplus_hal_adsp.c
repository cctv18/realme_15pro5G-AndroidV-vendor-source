...
static int oplus_chg_8350_set_otg_src_pdo(struct oplus_chg_ic_dev *ic_dev, int curr_ma)
{
	struct battery_chg_dev *bcdev;
	struct psy_state *pst = NULL;
	int rc = 0;
	...
	pst = &bcdev->psy_list[PSY_TYPE_USB];
	rc = write_property_id(bcdev, pst, USB_OTG_SRC_PDO, curr_ma);
	if (rc) {
		chg_err("set_otg_src_pdo %d mA failed, rc=%d\n", curr_ma, rc);
		return rc;
	}
	chg_info("set_otg_src_pdo: %d mA \n", curr_ma);
	return rc;
}
...
static void *oplus_chg_8350_buck_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	...
	case OPLUS_IC_FUNC_SET_OTG_BOOST_CURR_LIMIT:
		...
		break;
	case OPLUS_IC_FUNC_SET_OTG_SRC_PDO:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SET_OTG_SRC_PDO, oplus_chg_8350_set_otg_src_pdo);
		break;
