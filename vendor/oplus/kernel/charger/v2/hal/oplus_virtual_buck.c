...
static int oplus_chg_vb_set_otg_src_pdo(struct oplus_chg_ic_dev *ic_dev, int curr_ma)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
	int rc = 0;
	...
	for (i = 0; i < vb->child_num; i++) {
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_SET_OTG_SRC_PDO)) {
				rc = -ENOTSUPP;
				continue;
			}
			rc = oplus_chg_ic_func(
...
static void *oplus_chg_vb_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	...
	case OPLUS_IC_FUNC_SET_OTG_SRC_PDO:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SET_OTG_SRC_PDO, oplus_chg_vb_set_otg_src_pdo);
		break;
