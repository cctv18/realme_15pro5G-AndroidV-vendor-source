...
static void sih688x_f0_cali(sih_haptic_t *sih_haptic)
{
	int32_t tmp = 0;
	int32_t code = 0;
	int32_t tracking_f0_min = 0;
	int32_t tracking_f0_max = 0;

	if (DEVICE_ID_0815 == sih_haptic->device_id || DEVICE_ID_0809 == sih_haptic->device_id) {
		tracking_f0_min = SIH688X_F0_VAL_MIN;
		tracking_f0_max = SIH688X_F0_VAL_MAX;
	} else if (DEVICE_ID_1419 == sih_haptic->device_id) {
		tracking_f0_min = SIH688X_F0_VAL_MIN_1419;
		tracking_f0_max = SIH688X_F0_VAL_MAX_1419;
	} else {
		tracking_f0_min = SIH688X_F0_VAL_MIN;
		tracking_f0_max = SIH688X_F0_VAL_MAX;
	}
	hp_info("f0_min = %d, f0_max = %d\n", tracking_f0_min, tracking_f0_max);

	if (sih_haptic->detect.tracking_f0 > tracking_f0_max ||
		sih_haptic->detect.tracking_f0 < tracking_f0_min) {
		code = 0;
	} else {
...
static void sih688x_get_vbat(sih_haptic_t *sih_haptic)
{
	...
	hp_info("%s: detect_vbat:%d\n", __func__, sih_haptic->detect.vbat);
}

static void __maybe_unused sih688x_vbat_comp(sih_haptic_t *sih_haptic)
{
	...
	uint32_t drv_bst)
{
	...
	*
	*/
	if (drv_bst & SIH688X_MAX_VOUT)
		drv_bst = 0x78;
	if (drv_bst < SIH688X_BST_VOUT_6V)
		drv_bst = SIH688X_BST_VOUT_6V;
	bst_reg_val = (uint8_t)drv_bst;
	//bst_reg_val = (uint8_t)(((drv_bst - SIH688X_DRV_BOOST_BASE) *
	//	SIH688X_DRV_BOOST_SETP_COE) / SIH688X_DRV_BOOST_SETP);
	drv_bst = (bst_reg_val * SIH688X_DRV_BOOST_SETP / SIH688X_DRV_BOOST_SETP_COE) +
		SIH688X_DRV_BOOST_BASE;
	haptic_regmap_write(sih_haptic->regmapp.regmapping,
	...
	if (drv_bst >= SIH_ANA_CTRL_BST_LEVEL_6 &&
		drv_bst <= SIH_ANA_CTRL_BST_LEVEL_7) {
		haptic_regmap_update_bits(sih_haptic->regmapp.regmapping,
			SIH688X_REG_ANA_CTRL2, SIH_ANA_CTRL2_BST_EA_SEL_O_MASK,
			SIH_ANA_CTRL2_BST_EA_6_7);
	} else if (drv_bst > SIH_ANA_CTRL_BST_LEVEL_7 &&
		drv_bst <= SIH_ANA_CTRL_BST_LEVEL_9) {
		haptic_regmap_update_bits(sih_haptic->regmapp.regmapping,
			SIH688X_REG_ANA_CTRL2, SIH_ANA_CTRL2_BST_EA_SEL_O_MASK,
			SIH_ANA_CTRL2_BST_EA_7_9);
	} else if (drv_bst > SIH_ANA_CTRL_BST_LEVEL_9 &&
		drv_bst <= SIH_ANA_CTRL_BST_LEVEL_11) {
		haptic_regmap_update_bits(sih_haptic->regmapp.regmapping,
			SIH688X_REG_ANA_CTRL2, SIH_ANA_CTRL2_BST_EA_SEL_O_MASK,
			SIH_ANA_CTRL2_BST_EA_9_11);
	}
}
...
static void sih688x_get_ram_data(sih_haptic_t *sih_haptic, uint8_t *buf, uint32_t size)
{
	i2c_read_bytes(sih_haptic, SIH688X_REG_RAMDATA, buf, size);
}
...
static void sih688x_set_ram_addr(sih_haptic_t *sih_haptic, uint32_t base_addr)
{
	uint8_t ram_addr[2] = {0, 0};

	ram_addr[0] = (uint8_t)SIH_RAM_ADDR_H(base_addr);
	ram_addr[1] = (uint8_t)SIH_RAM_ADDR_L(base_addr);
	haptic_regmap_bulk_write(sih_haptic->regmapp.regmapping,
		SIH688X_REG_RAMADDRH, SIH_I2C_OPERA_BYTE_TWO, ram_addr);
}

static void sih688x_set_ram_data(sih_haptic_t *sih_haptic, uint8_t *data,
    uint32_t len)
{
    int i = 0;
    int cnt = 0;
    ...
    while (i < len) {
        if ((len - i) <= SIH_RAMDATA_READ_SIZE)
            cnt = len - i;
        else
            cnt = SIH_RAMDATA_READ_SIZE;
        haptic_regmap_write(sih_haptic->regmapp.regmapping,
            SIH688X_REG_RAMDATA, cnt, data);
        i += cnt;
    }
}
...
static void sih688x_get_wav_main_loop(sih_haptic_t *sih_haptic)
{
	...
	sih_haptic->ram.main_loop = (uint32_t)reg_val;
}
...
static void sih688x_set_cont_para(sih_haptic_t *sih_haptic, uint8_t flag,
	uint8_t *data)
{
	uint8_t reg_addr_start = SIH688X_REG_SEQ0_T_DRIVER;
...
static void sih688x_get_detect_f0(sih_haptic_t *sih_haptic)
{
	...
	if (DEVICE_ID_0815 == sih_haptic->device_id || DEVICE_ID_0809 == sih_haptic->device_id) {
		sih_f0_pre_value = SIH_F0_PRE_VALUE;
	} else if (DEVICE_ID_1419 == sih_haptic->device_id) {
		sih_f0_pre_value = SIH_F0_PRE_VALUE_1419;
	} else {
		sih_f0_pre_value = SIH_F0_PRE_VALUE;
	}
	hp_info("sih_f0_pre_value = %d\n", sih_f0_pre_value);
...
static int sih688x_convert_level_to_vmax(struct vmax_map *map, int val)
{
	int i;
...
static void sih688x_init(sih_haptic_t *sih_haptic)
{
	...
	*/
	...
	if (DEVICE_ID_0815 == sih_haptic->device_id || DEVICE_ID_0809 == sih_haptic->device_id) {
		sih_haptic->detect.detect_f0 = SIH_F0_PRE_VALUE;
		sih_haptic->detect.tracking_f0 = SIH_F0_PRE_VALUE;
		sih_haptic->detect.cali_target_value = SIH_F0_TARGET_VALUE;
	} else if (DEVICE_ID_1419 == sih_haptic->device_id) {
		sih_haptic->detect.detect_f0 = SIH_F0_PRE_VALUE_1419;
		sih_haptic->detect.tracking_f0 = SIH_F0_PRE_VALUE_1419;
		sih_haptic->detect.cali_target_value = SIH_F0_TARGET_VALUE_1419;
	} else {
		sih_haptic->detect.detect_f0 = SIH_F0_PRE_VALUE;
		sih_haptic->detect.tracking_f0 = SIH_F0_PRE_VALUE;
		sih_haptic->detect.cali_target_value = SIH_F0_TARGET_VALUE;
	}
	hp_info("detect_f0=%d,tracking_f0=%d,cali_target_value=%d\n",
		sih_haptic->detect.detect_f0, sih_haptic->detect.tracking_f0,
		sih_haptic->detect.cali_target_value);
	...
	sih_haptic->chip_ipara.state = SIH_STANDBY_MODE;
	sih_haptic->chip_ipara.play_mode = SIH_IDLE_MODE;
	sih_haptic->rtp.audio_delay = 0;
    sih_haptic->osc_para.osc_data = 0;
...
haptic_func_t sih_688x_func_list = {
	...
	.set_ram_data = sih688x_set_ram_data,
