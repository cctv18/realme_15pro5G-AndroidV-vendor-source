			     ...
			     struct device_node *np)
{
	...
	if (of_property_read_u32(np, "aw8693xs_boost_voltage", &max_boost_voltage))
		aw_haptic->max_boost_vol = DEFAULT_BOOST_VOLT;
	else
...
#ifdef OPLUS_FEATURE_CHG_BASIC
static int aw8693xs_convert_level_to_vmax(struct vmax_map *map, int val)
{
	int i;
...
static void aw8693xs_haptic_value_init(struct aw_haptic *aw_haptic)
{
	...
	if (!aw_haptic->info.bemf_d2s_gain)
		aw_haptic->info.bemf_d2s_gain = AW8693XS_BEMF_D2S_GAIN;

	if (!aw_haptic->info.d2s_gain)
		aw_haptic->info.d2s_gain = AW8693XS_D2S_GAIN;
