...
#ifdef CONFIG_SND_SOC_OPLUS_PA_MANAGER
enum aw_customers_profile_id_pa_manager {
...
enum {
	...
	AW87XXX_VOLTAGE_LEVEL_2 = 1,
	AW87XXX_VOLTAGE_LEVEL_3 = 2,
	AW87XXX_VOLTAGE_LEVEL_4 = 3,
	AW87XXX_VOLTAGE_LEVEL_COUNT,
};

void *oplus_pa_aw87xxx_node_v2 = NULL;
...
#ifdef CONFIG_SND_SOC_OPLUS_PA_MANAGER
uint8_t aw_speaker_get_mode(struct oplus_speaker_device *speaker_device)
{
...
int aw87xxx_enable_pa(struct oplus_speaker_device *speaker_device, int enable)
{
	int ret = 0;
	int channel = speaker_device->type - L_SPK;
	int pa_mode = aw_speaker_get_mode(speaker_device);

	if(enable) {
		ret = aw87xxx_set_profile_by_id(channel, pa_mode);
	} else {
		ret = aw87xxx_set_profile_by_id(channel, AW_CTOS_PROFILE_OFF);
	}

	return ret;
}

int aw87xxx_get_pa_status(int dev_index)
{
	struct list_head *pos = NULL;
	struct aw87xxx *aw87xxx = NULL;
	int index = 0;
...
int aw87xxx_speaker_status_get(struct oplus_speaker_device *speaker_device)
{
	int status = 0;
	int dev_index = 0;
	...
	dev_index = speaker_device->type - L_SPK;
	status = aw87xxx_get_pa_status(dev_index);

	pr_info("%s, %d, dev_index = %d, status = %d\n", __func__, __LINE__, dev_index, status);

	return status;
}

#define VOL_STATUS_LOW 0
#define VOL_STATUS_HIGH 1

int aw87xxx_voltage_set(struct oplus_speaker_device *speaker_device, int level)
{
	int channel = speaker_device->type - L_SPK;
	if (level < 0 || level >=  AW87XXX_VOLTAGE_LEVEL_COUNT) {
		pr_err("%s, %d, boost level is invalid value\n", __func__, __LINE__);
		return -EINVAL;
	}

	if (level >= AW87XXX_VOLTAGE_LEVEL_3) {
		aw87xxx_set_boost_voltage(channel, VOL_STATUS_HIGH);
		aw87xxx_spk_low_voltage_status = VOL_STATUS_HIGH;
	} else {
		aw87xxx_set_boost_voltage(channel, VOL_STATUS_LOW);
...
int aw87xxx_voltage_get(struct oplus_speaker_device *speaker_device)
{
...
{
	int channel = speaker_device->type - L_SPK;
	uint8_t pa_mode = AW_CTOS_PROFILE_MUSIC;

	if (enable) {
		aw87xxx_set_profile_by_id(channel, AW_CTOS_PROFILE_OFF);
		is_mute_status = true;
	} else {
		pa_mode = aw_speaker_get_mode(speaker_device);
		aw87xxx_set_profile_by_id(channel, pa_mode);
		is_mute_status = false;
	}
}

void aw87xxx_speaker_mute_get(struct oplus_speaker_device *speaker_device, int *enable)
{
...
#endif
{
	...
	if (speaker_device == NULL || speaker_device->speaker_manufacture == MFR_AWINIC) {
		// Device
		speaker_device = kzalloc(sizeof(struct oplus_speaker_device), GFP_KERNEL);
		if (speaker_device != NULL) {
			speaker_device->speaker_manufacture = MFR_AWINIC;
			speaker_device->chipset = aw87xxx->aw_dev.chipid;
			speaker_device->type = (aw87xxx->dev_index == 0) ? L_SPK : R_SPK;
			speaker_device->speaker_enable_set = aw87xxx_enable_pa;
			speaker_device->speaker_enable_get = aw87xxx_speaker_status_get;
			speaker_device->boost_voltage_set = aw87xxx_voltage_set;
			speaker_device->boost_voltage_get = aw87xxx_voltage_get;
		...
		speaker_device->speaker_protection_set = aw87xxx_enable_pa;
		speaker_device->speaker_protection_get = aw87xxx_speaker_status_get;
	}
