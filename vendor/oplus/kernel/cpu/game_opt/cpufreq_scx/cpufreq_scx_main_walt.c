...
static void scx_irq_work(struct irq_work *irq_work)
{
	...
	for_each_sched_cluster(cluster) {
		...
		for_each_cpu(cpu, &cluster_online_cpus) {
			rq = cpu_rq(cpu);
#ifdef CONFIG_HMBIRD_SCHED
			scx_update_task_ravg(rq->curr, rq, TASK_UPDATE, wc);
#else
	...
	if ((flags & WALT_CPUFREQ_ROLLOVER_BIT) && !(flags & WALT_CPUFREQ_CONTINUE_BIT)) {
		...
		for_each_cpu(cpu, policy->cpus) {
			...
			if (get_hmbird_rq(rq)->exclusive) {
				exclusive = max(exclusive, wrq->prev_runnable_sum_fixed);
				continue;
			}

			prev = wrq->prev_runnable_sum_fixed;
			sum += prev;
			max = max(max, prev);
			nr++;
		}
		...
		scx_gov_update_cpufreq(policy, max(exclusive, avg));
	}
}
#else
...
{
	struct scx_gov_cpu *sg_cpu = container_of(cb, struct scx_gov_cpu, cb);
	struct cpufreq_policy *policy = sg_cpu->sg_policy->policy;
	u64 exclusive = 0;
	u64 sum = 0, avg = 0, nr = 0, max = 0, prev;
	int cpu, cluster_id;
	if ((flags & WALT_CPUFREQ_ROLLOVER_BIT) && !(flags & WALT_CPUFREQ_CONTINUE_BIT)) {
