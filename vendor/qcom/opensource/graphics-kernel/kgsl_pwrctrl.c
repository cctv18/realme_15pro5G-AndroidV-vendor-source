			...
			struct kgsl_pwr_constraint *pwrc, uint32_t id, u32 ts)
{
	...
	if ((pwrc_old->type == KGSL_CONSTRAINT_NONE) ||
		...
		if (constraint != 0) {
			dev_warn(device->dev, "kgsl_constraint:constraint_value=%u status=1", constraint);
		}
...
static int pmqos_max_notifier_call(struct notifier_block *nb, unsigned long val, void *data)
{
	...
	if (max_freq < pwr->pwrlevels[0].gpu_freq) {
		dev_warn(device->dev,"kgsl_thermal_constraint:max_freq=%u", max_freq);
	}
#endif /* OPLUS_GPU_CONSTRAINT_LOG */
		...
		unsigned long state)
{
	...
	/* Only print when constraint is lower than highest freq */
	if (state > 0) {
		dev_warn(device->dev,"kgsl_thermal_constraint:cur_state=%u", freq);
	}
#endif /* OPLUS_GPU_CONSTRAINT_LOG */
