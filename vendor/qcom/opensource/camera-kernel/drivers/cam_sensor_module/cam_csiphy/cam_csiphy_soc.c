...
#include "include/cam_csiphy_2_2_1_hwreg_khFront.h"
#endif
	...
	struct csiphy_device *csiphy_dev)
{
	...
	if (of_device_is_compatible(soc_info->dev->of_node, "qcom,csiphy-v2.1.0")) {
		...
		csiphy_dev->is_divisor_32_comp = true;
		csiphy_dev->clk_lane = 0;
	} else if (of_device_is_compatible(soc_info->dev->of_node, "qcom,csiphy-khFront")) {
		csiphy_dev->ctrl_reg = &ctrl_reg_2_2_1_khFront;
		csiphy_dev->hw_version = CSIPHY_VERSION_V221_khFront;
		csiphy_dev->is_divisor_32_comp = true;
		csiphy_dev->clk_lane = 0;
#endif
	} else {
