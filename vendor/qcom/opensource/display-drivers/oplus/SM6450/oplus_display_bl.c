...
static u32 custom_cmd_set;
static bool oplus_disable_backlight = false;
...
int oplus_panel_parse_bl_cfg(struct dsi_panel *panel)
{
	...
	panel->oplus_panel.bl_cfg.bl_demura_compatible_dbv_threshold_support = utils->read_bool(utils->data,
			"oplus,bl_demura-compatible-dbv-threshold-enable");
	OPLUS_DSI_INFO("oplus,bl_demura-compatible-dbv-threshold-enable: %s\n",
			panel->oplus_panel.bl_cfg.bl_demura_compatible_dbv_threshold_support ? "true" : "false");

	panel->oplus_panel.bl_cfg.bl_demura_send_async_support = utils->read_bool(utils->data,
			"oplus,bl_demura_send_async");
	OPLUS_DSI_INFO("oplus,bl_demura_send_async: %s\n",
			panel->oplus_panel.bl_cfg.bl_demura_send_async_support ? "true" : "false");

	if (panel->oplus_panel.bl_cfg.bl_demura_send_async_support) {
		panel->oplus_panel.bl_cfg.oplus_panel_backlight_demura_send_async_wq = create_singlethread_workqueue("oplus_panel_backlight_demura_send_async_wq");
		INIT_WORK(&panel->oplus_panel.bl_cfg.oplus_panel_backlight_demura_send_async_work, oplus_panel_backlight_demura_send_async_handler);
	}
...
void oplus_panel_backlight_demura_send_async_handler(struct work_struct *work)
{
	int rc = 0;
	...
	OPLUS_DSI_TRACE_BEGIN("oplus_panel_backlight_demura_send_async_handler");
	mutex_lock(&panel->panel_lock);
	rc = dsi_panel_tx_cmd_set(panel, custom_cmd_set, false);
	mutex_unlock(&panel->panel_lock);
	OPLUS_DSI_TRACE_END("oplus_panel_backlight_demura_send_async_handler");

	return;
}

void oplus_panel_backlight_compatible_demura_dbv_threshold(struct dsi_panel *panel, u32 backlight_level, u32 bl_demura_mode)
{
	int rc = 0;
	u8 cmd;
	...
	oplus_panel_cmd_reg_replace(panel, custom_cmd_set, cmd, replace_reg, replace_reg_len);

	if (panel->oplus_panel.bl_cfg.bl_demura_send_async_support) {
		queue_work(panel->oplus_panel.bl_cfg.oplus_panel_backlight_demura_send_async_wq, &panel->oplus_panel.bl_cfg.oplus_panel_backlight_demura_send_async_work);
	} else {
		rc = dsi_panel_tx_cmd_set(panel, custom_cmd_set, false);
	}
	return;
...
void oplus_panel_backlight_demura_dbv_switch(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	} else if (!strcmp(panel->name, "AC292 P 7 A0025 vid mode panel")) {
		if ((bl_lvl >= 8) && (bl_lvl <= 1153)) {
			panel->oplus_panel.bl_demura_mode = 0;
			bl_demura_mode = DSI_CMD_DEMURA_DBV_MODE0;
		} else if ((bl_lvl >= 1154) && (bl_lvl <= 4094)) {
			panel->oplus_panel.bl_demura_mode = 1;
			bl_demura_mode = DSI_CMD_DEMURA_DBV_MODE1;
		}
	} else if (!strcmp(panel->name, "AC292 P 3 A0025 vid mode panel")) {
		if ((bl_lvl >= 1) && (bl_lvl <= 1147)) {
			panel->oplus_panel.bl_demura_mode = 0;
			bl_demura_mode = DSI_CMD_DEMURA_DBV_MODE0;
		} else if ((bl_lvl >= 1148) && (bl_lvl <= 4094)) {
			panel->oplus_panel.bl_demura_mode = 1;
			bl_demura_mode = DSI_CMD_DEMURA_DBV_MODE1;
		}
	} else {
	...
	if ((panel->oplus_panel.bl_demura_mode != bl_demura_last_mode || oplus_last_backlight == 0 || oplus_last_backlight == 1)
		&& (panel->power_mode == SDE_MODE_DPMS_ON)) {
		if (panel->oplus_panel.bl_cfg.bl_demura_compatible_dbv_threshold_support) {
			oplus_disable_backlight = true;
			oplus_panel_backlight_compatible_demura_dbv_threshold(panel, bl_lvl, bl_demura_mode);
		} else {
		...
		struct mipi_dsi_device *dsi, u32 bl_lvl)
{
	...
	if (oplus_disable_backlight) {
		oplus_disable_backlight = false;
	} else {
...
int oplus_ofp_video_mode_ramless_lhbm_pressed_icon_backlight_gamma_update(void *dsi_panel, enum dsi_cmd_set_type type)
{
	int bl_level;
	...
	if (!oplus_ofp_local_hbm_is_enabled()) {
		OFP_DEBUG("local hbm is not enabled, no need to update lhbm gamma\n");
		return 0;
	}

	OPLUS_OFP_TRACE_BEGIN("oplus_ofp_video_mode_ramless_lhbm_pressed_icon_gamma_update");

	bl_level = panel->bl_config.bl_level;
	if (bl_level) {
		if (type == DSI_CMD_LHBM_PRESSED_ICON_ON) {
			if (bl_level <= 1154) {
				type = DSI_CMD_LHBM_ICON_ON_GAMA_UPDATE0;
			} else if ((bl_level <= 1870) && (bl_level > 1154)) {
				type = DSI_CMD_LHBM_ICON_ON_GAMA_UPDATE1;
			} else if ((bl_level <= 2838) && (bl_level > 1870)) {
				type = DSI_CMD_LHBM_ICON_ON_GAMA_UPDATE2;
			} else if ((bl_level <= 3515) && (bl_level > 2838)) {
				type = DSI_CMD_LHBM_ICON_ON_GAMA_UPDATE3;
			} else {
				type = DSI_CMD_LHBM_ICON_ON_GAMA_UPDATE4;
			}
		} else if (type == DSI_CMD_LHBM_PRESSED_ICON_OFF) {
			if ((bl_level <= 1153) && (bl_level > 8)) {
				type = DSI_CMD_LHBM_ICON_OFF_GAMA_UPDATE0;
			} else {
				type = DSI_CMD_LHBM_ICON_OFF_GAMA_UPDATE1;
			}
	  	}
	  }
	OFP_DEBUG("update lhbm pressed icon gamma successfully\n");

	OPLUS_OFP_TRACE_END("oplus_ofp_video_mode_ramless_lhbm_pressed_icon_gamma_update");
