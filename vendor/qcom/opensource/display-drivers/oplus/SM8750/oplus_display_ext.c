...
int oplus_panel_switch_smera_mode(struct drm_encoder *drm_enc)
{
	struct sde_encoder_virt *sde_enc = NULL;
	struct sde_encoder_phys *phys_encoder = NULL;
	...
	u32 rc = 0;
	int refresh_rate = 0;
	unsigned int delay_frames = 2;
	static int last_refresh_rate = 0;
	static unsigned int te_count = 0;
	static int dsi_cmd_smear_mode_switch = 0;
	...
	if (display->panel->power_mode == SDE_MODE_DPMS_OFF || oplus_sync_power_state == SDE_MODE_DPMS_LP1) {
		return EFAULT;
	}

	if ((refresh_rate == 60 && last_refresh_rate != 60) ||
		(refresh_rate != 60 && last_refresh_rate == 60) ||
		(te_count < delay_frames && te_count > 0)) {
		if (te_count == 1 && refresh_rate != last_refresh_rate){
			te_count = 1;
		} else {
			te_count++;
		}
	...
	if ((te_count == delay_frames) && (display->panel->power_mode != SDE_MODE_DPMS_OFF || oplus_sync_power_state != SDE_MODE_DPMS_LP1)) {
		if (refresh_rate == 120 || refresh_rate == 90 || refresh_rate == 144) {
			dsi_cmd_smear_mode_switch = DSI_CMD_SMEAR_MODE0;
		} else if (refresh_rate == 60) {
			dsi_cmd_smear_mode_switch = DSI_CMD_SMEAR_MODE1;
		} else {
			return EFAULT;
		}

		SDE_ATRACE_BEGIN("oplus_panel_switch_smera_mode");

		atomic_set(&display->panel->oplus_panel.esd_pending, 1);
		OPLUS_DSI_ERR("[%s] send smera cmd, skip esd\n", __func__);

		mutex_lock(&display->panel->panel_lock);
		rc = dsi_panel_tx_cmd_set(display->panel, dsi_cmd_smear_mode_switch, false);
		mutex_unlock(&display->panel->panel_lock);
		if (rc) {
			OPLUS_DSI_ERR("[%s] failed to send DSI_CMD_SMEAR_MODE cmds, rc=%d\n",
				display->panel->name, rc);
		}

		atomic_set(&display->panel->oplus_panel.esd_pending, 0);

		SDE_ATRACE_END("oplus_panel_switch_smera_mode");
