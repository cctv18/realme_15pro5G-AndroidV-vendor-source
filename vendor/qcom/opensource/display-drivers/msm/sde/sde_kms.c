...
#define GET1_H32(val) (val >> 32)
#define GET1_L32(val) (val & 0xffffffff)
...
static void sde_kms_handle_power_event(u32 event_type, void *usr)
{
	...
	qtimer = arch_timer_read_counter();
	SDE_EVT32(event_type, GET1_H32(qtimer), GET1_L32(qtimer));

	slice = llcc_slice_getd(sde_kms->catalog->sc_cfg[SDE_SYS_CACHE_DISP].llcc_uid);
	if (IS_ERR_OR_NULL(slice))  {
		SDE_ERROR("failed to get llcc slice for uid:%d\n",
		sde_kms->catalog->sc_cfg[SDE_SYS_CACHE_DISP].llcc_uid);
	} else {
	...
	if (event_type == SDE_POWER_EVENT_POST_ENABLE) {
		...
		if (!IS_ERR_OR_NULL(slice) && !sde_in_trusted_vm(sde_kms)) {
			llcc_slice_activate(slice);
			SDE_EVT32(event_type, scid);
		}
